...
public class KaropapierLoader
{
	...
	public static Karopapier initiate(Profile profile)
	{
		try
		{
			if(login(profile))
			{
				return initiateKaropapier();
			}
			else
				return null;
		}
			...
			throws MalformedURLException, IOException
	{
		...
		return new Karopapier(maps, players, currentUser);
	}
	...
	public static boolean login(Profile profile) throws IOException
	{
		return KaropapierLoader.login(profile.getKaroUsername(), profile.getKaroPassword());
	}

	public static boolean login(String username, String password) throws IOException
	{
		System.out.print("Melde user an: \"" + username + "\" ... ");
	...
	public static Karopapier debugInstance()
	{
		TreeMap<Integer, Map> maps = new TreeMap<Integer, Map>();
		for(int i = 0; i < 100; i++)
		{
